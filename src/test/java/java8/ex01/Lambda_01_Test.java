...
public class Lambda_01_Test {
    ...
    private List<Person> filter(List<Person> persons, PersonPredicate predicate) {
    	...
    	List<Person> listTrie = new ArrayList<>();
    	for (int i = 0; i < persons.size(); i++) {
			if (predicate.test(persons.get(i))) {
				listTrie.add(persons.get(i));
			}
		}
    ...
    public void test_filter_by_password() throws Exception {
        ...
        List<Person> result = filter(personList, p -> p.getAge() >49 & StringUtils.equals(passwordSha512Hex, DigestUtils.sha512Hex(p.getPassword())));
